...
public interface CountriesRepo extends CrudRepository<Countries, Long> {
    ...
    List<Object[]> getallcountries();
         ...
         @Query(value="select A.name,count(A.Code) AS NumberOfCities from countries AS A join cities AS B on A.code = B.country_code group by A.code order by NumberOfCities desc ", nativeQuery=true)
